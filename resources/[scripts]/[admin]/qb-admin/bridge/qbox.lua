...
if (IsDuplicityVersion()) then
    ...
    Bridge = {
        ...
        GetOfflinePlayerFromCharacterIdentifier = function(CitizenId)
            ...
            for _, playerId in pairs(GetPlayers()) do
                ...
                if Player then
                    ...
                    if exports.qbx_core:HasPermission(playerId, 'god') then
                        playerRole = 'god'
                    elseif exports.qbx_core:HasPermission(playerId, 'admin') then
                        playerRole = 'admin'
                    end
        ...
        GetPlayerRole = function(targetId) return exports.qbx_core:GetPermission(targetId) end, GetCharacterName = function(targetId)
...
if not IsDuplicityVersion() then
    RegisterNetEvent('919-admin:client:spawnVehicle', function(model, plate, props)
        ...
        if not model or model == "" then
            print("^1[919ADMIN-Bridge] ERROR: No model provided^0")
            return
        end

        if not IsModelInCdimage(model) or not IsModelAVehicle(model) then
            print(("^1[919ADMIN-Bridge] Invalid vehicle model: %s^0"):format(model))
            return
        end
        ...
        if veh and veh ~= 0 then
            ...
            print("^2[919ADMIN-Bridge] Vehicle spawned successfully: " .. tostring(model) .. "^0")
        else
            print("^1[919ADMIN-Bridge] ERROR: Failed to create vehicle entity^0")
        end
    end)
