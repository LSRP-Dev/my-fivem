...
function RemoveTargetZone(zoneName)
    local zoneId = "casinoSystem-"..zoneName
    if Config.TargetScript == "ox-target" or Config.TargetScript == "ox_target" then
        -- Suppress warning when removing zones that may not exist (cleanup scenarios)
        exports[Config.TargetScript]:removeZone(zoneId, true)
    else
        exports[Config.TargetScript]:RemoveZone(zoneId)
    end
end
