...
function DisplayStore(index)
    ...
    for i=1, #store.catalog do 
        ...
        local description
        if not item.required or #item.required > 1 or (item.required[1].type ~= "cash" and not Inventory.Items[item.name]) then
            description = item.description
            if item.required[1].type == "cash" then
                description = "$" .. item.required[1].amount .. " - " .. item.description
            else
                description = Inventory.Items[item.name].label .. " - " .. item.description
            end
        end
        local option = {
